...
当宿主支持原生选择 UI 时，这个 skill 会优先使用原生选择，而不是普通 prose。对 Codex 来说，就是在可用时使用 `request_user_input`；对 Claude Code 或 gstack 风格宿主来说，就是在可用时使用真实的 `AskUserQuestion` 工具。详细宿主映射放在 `brainstorming-only/references/user-choice-output-protocol.md`。普通 A/B/C 文本只作为 fallback。
